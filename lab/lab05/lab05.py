...
def distance(city_a, city_b):
    ...
    dis = sqrt(abs(lat_a-lat_b)**2 + abs(lon_a-lon_b)**2)
    return dis
...
def closer_city(lat, lon, city_a, city_b):
    ...
    """
    "*** YOUR CODE HERE ***"
    current_dis = make_city("current",lat,lon)

    return get_name(city_a) if distance(current_dis,city_a) < distance(current_dis,city_b) else get_name(city_b)
...
def berry_finder(t):
    ...
    if label(t) == 'berry':
        ...
        for i in branches(t):
            result = result or berry_finder(i)
    return result
...
def coords(fn, seq, lower, upper):
    ...
    """
    "*** YOUR CODE HERE ***"
    return [ [i,fn(i)] for i in seq if fn(i) >=1 and fn(i) <=9 ]
