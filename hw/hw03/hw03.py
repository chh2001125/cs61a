...
def pingpong(n):
    ...
    """
    "*** YOUR CODE HERE ***"
    def helper(index,ppn,dire):
        if index == n:
            return ppn
        elif index % 8 == 0 or bool(num_eights(index)):
            return helper(index+1,ppn-dire,-dire)
        else:
            return helper(index+1,ppn+dire,dire)
    return helper(1,1,1)
...
def count_coins(change):
    ...
    def count_partitions(amount,smallest_coin):
        ...
        if smallest_coin == 25:
            ...
            return with_coin
        without_coin = count_partitions(amount,get_next_coin(smallest_coin))
        with_coin = count_partitions(amount-smallest_coin,smallest_coin)
